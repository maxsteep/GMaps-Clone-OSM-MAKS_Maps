...
#define T_AREA_HEIGHT	1  /* Height of text window */
...
/* Return 1 if I it can be guaranteed that no part of this rectangle will*
 * be within the user drawing area.  Otherwise return 0.                 *
...
drawellipticarc(float xc, float yc, float radx, float rady, float startang, float angextent) {
    ...
    if (gl_state.disp_type == SCREEN) {
        float screen_rad_x = fabs(xworld_to_scrn(radx) - xworld_to_scrn(0));
        float screen_rad_y = fabs(yworld_to_scrn(rady) - yworld_to_scrn(0));
...
void drawtext_left(float xc, float yc, const std::string& str_text, float boundx, float boundy) {
    ...
    //experimenting with off screen draw
    if (rect_off_screen(t_bound_box(t_point(xc - zero_approx_width / 2, yc - zero_approx_height / 2), zero_approx_width, zero_approx_height))) {
        // if the largest bbox the text could have is off the screen, don't even load the font.
        return;
    }
#elif defined WIN32
    ...
    //experimenting with off screen draw
    if (rect_off_screen(t_bound_box(t_point(xc - width / 2, yc - height / 2), width, height))) {
        // if the largest bbox the text could have is off the screen, don't even load the font.
        return;
    }
#else /* WC : WIN32 */
...
int get_text_length(const std::string& textstring) {
...
void draw_surface(const Surface& surface, double x, double y) {
    if (rect_off_screen(x, y + 0.00000001, x, y + 0.00000001)) return;
...
static void x11_init_graphics(const char *window_name) {
    ...
    unsigned int border_width = 0; /* ignored by OpenWindows */  //This used to be 2
...
//    trans_coord.top_height = 4 * trans_coord.display_height / 5;
      ...
      trans_coord.top_width = trans_coord.display_width;                              //New modify here
      trans_coord.top_height = trans_coord.display_height;
...
#define ON 0
    ...
    x11_turn_on_off(ON);
    while (!exitEvent) {
//        if (keyboardInput.compare("proceed") == 0) return;
